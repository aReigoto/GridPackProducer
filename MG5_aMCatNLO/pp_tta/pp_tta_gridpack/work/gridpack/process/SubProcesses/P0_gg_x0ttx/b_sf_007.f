...
C     ----------
     ...
     $   //' only with calculatedborn = true'
      ...
      ELSEIF (CALCULATEDBORN) THEN
        ...
        ENDDO
      ENDIF
      JAMP1(1)=+IMAG1*AMP(1)+IMAG1*AMP(2)-AMP(3)-AMP(4)-AMP(8)
      JAMP1(2)=-IMAG1*AMP(1)-IMAG1*AMP(2)-AMP(5)-AMP(6)-AMP(7)
      JAMP2(1)=+1D0/2D0*(+1D0/3D0*IMAG1*AMP(1)+1D0/3D0*IMAG1*AMP(2)
     $ -1D0/3D0*AMP(3)-1D0/3D0*AMP(4)-1D0/3D0*AMP(8))
      JAMP2(2)=+1D0/2D0*(-1D0/3D0*IMAG1*AMP(1)-1D0/3D0*IMAG1*AMP(2)
     $ -1D0/3D0*AMP(5)-1D0/3D0*AMP(6)-1D0/3D0*AMP(7))
      JAMP2(3)=+1D0/2D0*(+AMP(3)+AMP(4)+AMP(5)+AMP(6)+AMP(7)+AMP(8))
      B_SF_007 = 0.D0
      DO I = 1, NCOLOR1
        ZTEMP = (0.D0,0.D0)
        DO J = 1, NCOLOR2
