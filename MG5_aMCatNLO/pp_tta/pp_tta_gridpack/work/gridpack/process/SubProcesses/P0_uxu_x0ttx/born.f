...
C     
C     Process: u~ u > x0 t t~ QNP<=1 WEIGHTED<=4 [ all = QCD ]
C     Process: c~ c > x0 t t~ QNP<=1 WEIGHTED<=4 [ all = QCD ]
C     Process: d~ d > x0 t t~ QNP<=1 WEIGHTED<=4 [ all = QCD ]
C     Process: s~ s > x0 t t~ QNP<=1 WEIGHTED<=4 [ all = QCD ]
C     
      IMPLICIT NONE
C     
C     CONSTANTS
...
C     GLOBAL VARIABLES
C     
      DOUBLE PRECISION AMP2(2), JAMP2(0:2)
      ...
      DO IHEL=BACK_HEL,-BACK_HEL,STEP_HEL
        IF (IHEL.EQ.BACK_HEL.OR.NHEL(GLU_IJ).NE.0) THEN
          ...
          IF (.NOT. CALCULATEDBORN) THEN
            ...
            CALL FFS1_5_1(W(1,4),W(1,3),GC_3010H,GC_3010A,MDL_MT
     $       ,MDL_WT,W(1,2))
C           Amplitude(s) for diagram number 1
            CALL FFV1_0(W(1,5),W(1,2),W(1,6),GC_5,AMP(1))
            CALL FFS1_5_2(W(1,5),W(1,3),GC_3010H,GC_3010A,MDL_MT
     $       ,MDL_WT,W(1,2))
C           Amplitude(s) for diagram number 2
            CALL FFV1_0(W(1,2),W(1,4),W(1,6),GC_5,AMP(2))
            DO I=1,NGRAPHS
